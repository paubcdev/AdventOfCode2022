...
fn solve_part_1(input: &str) -> usize {
    let grid: Vec<Vec<char>> = input.lines().map(|line| line.chars().collect::<Vec<char>>())
        .collect();
    ...
    let final_result = result.expect("no path found").len() -1;
...
abdefghi";
    ...
    #[test]
    fn test_part_1() {
        assert_eq!(solve_part_1(DATA), 31);
    }

    #[test]
    fn test_part_2() {
        assert_eq!(solve_part_2(DATA), 29);
    }
}